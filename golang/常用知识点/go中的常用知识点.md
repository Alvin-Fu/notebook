...
- 加权轮询法：根据服务器的性能给每个服务器有不同的权重

使用hash进行均衡的问题：
1. 当一个服务器宕机或者添加一个服务器的时候这时候需要重新计算hash值，在缓存系统中这时的所有请求都将涌向数据库服务，导致雪崩
2. 当一个服务器性能提升或者下降的时候，hash算法不能动态的让这个服务少分担或者多分担负载
为了解决这个问题引入了一致性哈希算法
#### 一致性哈希(consistent hashing)
hash算法和单调性，单调性是指如果已经有一些内容通过哈希分派到了相应的缓冲中，当有新加入的时候，hash的结果应该能够保证原有已分配的内容可以被映射到新的缓冲中去，而不会被映射到旧的缓冲集合中的其他缓冲区
**consistent hashing算法的原理**：在一致性哈希中就是当添加或者移除一个cache的时候，它可以尽可能小的改变已存在key的映射关系
在consistent hashing中为了保证平衡性，引入了虚拟节点，到对象被映射到虚拟节点的时候这些虚拟节点会到对应的真实节点

![引入虚拟节点](../../.local/static/2020/5/0/consistent.1591523771146.png)
...
通过LVS可以达到的技术目标：通过LVS达到负载均衡技术和Linux操作系统实现一个高性能高可用的Linux服务器集群，具有良好的可靠性、可扩展性和可操作性。使用低廉的成本获得最优的性能
LVS是一个实现负载均衡集群的开源项目，LVS架构从逻辑上可分为调度层，Server集群层和共享存储
- 
基本工作原理

![LVS原理图](../../.local/static/2020/5/0/LVS.1591526911492.png)

1. 当用户向负载均衡调度器发起请求，调度器讲请求发往至内核空间
...
### 16、微服务架构是什么样子的
微服务是为了解决传统服务过于庞大并且维护成本高的问题	
一般来讲微服务可以理解为将大型项目拆分成若干个小模块，可以进行快速迭代更新线上的需求。一般新的功能模块会构成新的服务组件，与其他以发布的服务组件以同协同。服务内部有多个生产者和消费者，通常以http rest的方式调用，但是对于客户来说还是以一个整体呈现的
微服务架构: 采用一组服务的方式来构建一个应用，服务独立部署在不同的进程中，不同服务通过一些轻量级交互机制来通信，如：RPC，HTTP等，服务可独立扩展伸缩，每个服务定以了明确的边界，不同的服务甚至可以采用不同的编程语言来实现，由独立的团队进行维护	
...
根据分布式的CAP理论可以了解到任何一个分布式系统都不能同时满足，一致性，可用性和分区容错性，最多只能满足两项。因此在大多数的场景中都是牺牲了强一致性来换取系统的高可用性，系统往往只需要保证“最终一致性”
...
#### 基于数据库的实现方式
在数据库中建立一张表，保存方法名等信息，使用方法名作为唯一索引，在执行方法的时候使用方法名向数据库中插入数据，成功则获取锁，执行完成后删除对应的数据释放锁
需要解决和优化的问题：

- 数据库的性能，可用性直接影响到，因此需要数据库进行双机部署，数据同步，主备切换
- 不具备可重入特性，需要在表中增加获取锁的机器和线程的信息，再次获取锁的时候，可以查询表中信息是否和当前信息相同，相同则直接获取锁
- 没有锁失效机制，一般需要有定时任务去清理
- 不具备锁的特性，获取不到就会返回失败，所以需要优化或缺逻辑，循环多次去获取
- 依赖数据库需要一定的资源开销，性能问题需要考虑 

#### 基于缓存的实现方式
基于redis的实现方式
选用Redis实现分布式锁原因：**redis有很高的性能**， **redis的命令对此支持较好，实现起来比较方便**
实现方式

在使用redis的过程中如果使用单点可能会导致服务的可用性变差，一般在稳定性要求较高的场合官方建议使用redis集群(获得多数机器的认同表示获取到锁)
优点：性能高，redis可持久化，也能保证数据不易丢失，redis集群方式提高稳定性
缺点：使用redis主从切换时可能丢失部分数据

#### 基于zookeeper的实现
zk是一个为分布式应用提供一致性服务的开源组件，它内部是一个分层的文件系统目录树结构，规定同一个目录下只能有一个唯一文件名
实现步骤：
	1. 创建一个目录myLock
	2. 线程A想获得锁就在myLock目录下创建临时顺序节点
	3. 获取mylock目录下所有的子节点，然后获取比自己小的兄弟节点，如果不存在，则说明当前线程顺序号最小，获得锁
	4. 线程B获取所有节点，判断自己不是最小节点，设置监听比自己次小的节点
	5. 线程A处理完，删除自己的节点，线程B监听到变更事件，判断自己是不是最小的节点，如果则获取锁

开源库Curator中提供的InterProcessMutex是分布式锁的实现，qcquire方法用于获取锁，release方法用于释放锁
优点：具备高可用，可重入，阻塞锁特性，可解决失效死锁问题
缺点：由于需要频繁的创建和删除节点，性能上不如redis方式
