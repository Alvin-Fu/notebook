...
epoll在系统初始化的时候会开辟自己的高速cache区，用于存放我们想要监控的socket，这些socket使用红黑树的形式被存放在内核cache层，支持快速查找，删除，增加操作。这个高速cache区，就是建立连续的物理内存页，然后在之上建立slab层，通常来讲，在物理上分配上你想要的size的内存，当使用的时候就在上班
