...
函数符号表，文件名表，函数结构和由偏移量引用的数据的内存都是连续的并且记录为一个单一的符号pclntab。最终地址被记录为epclntab。为了方便查找，大多数文件格式中符号本身都有一个对象文件节。pclntab的第一个字是一个魔术字一个uint32 0xfffffffb，后面跟随的是两个0字节，后面跟随的是一个字节给出了指令的大小量
