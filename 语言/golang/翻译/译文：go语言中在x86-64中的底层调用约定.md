...
异常的抛出使用的panic捕获使用defer和recover。panic被转换为一个运行时的内部方法进行常规调用的。这个内部方法是负责栈展开。编译器是知道这个panic是没有返回值得，跳过了为返回路径生成代码。异常捕获的机制完全隐藏在recover中，不需要堆代码生成器做特殊处理。代码生成对于可能会
